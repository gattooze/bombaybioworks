...
/* ============================================================================
   Hivemind — Collective intelligence & note-taking for Bombay Bioworks
   One core entity: a NOTE. Each note logs a conversation (or a standalone
   thought) with a TYPE (in person / call / video / email / message / event / note),
   participants, rich text, an auto-generated summary, tags and attachments.
   Storage: local IndexedDB by default, optional shared Supabase cloud sync.
   ========================================================================== */
...
/* Interaction types — answers "what form of contact was this?" */
const TYPES = [
...
/* ---- text helpers: strip HTML + extractive auto-summary ---- */
...
function autoSummary(html, maxSentences=2) {
  ...
  const scored = sentences.map((s,i) => {
    ...
    const score = words.reduce((a,w)=>a+(freq[w]||0),0) / (words.length||1) * (i===0?1.15:1); // slight lead bias
    return { s, score, i };
  });
...
/* ----------------------------------------------------------------------------
   Local storage adapter — IndexedDB
---------------------------------------------------------------------------- */
const STORES = ['people', 'notes', 'files'];
function openDB() {
  return new Promise((resolve, reject) => {
    const req = indexedDB.open('hivemind', 1);
...
/* ---- People picker (shared by note form) ---- */
...
function NoteForm({ note, onSave, onClose, defaultType }) {
  ...
  return <form onSubmit={submit} className="space-y-3">
    ...
    <Field label="Summary" right={<button type="button" onClick={()=>setF(s=>({...s,summary:autoSummary(s.body),summaryEdited:false}))} className="text-honey-600 hover:underline font-medium">✨ Auto-summarise</button>}
           hint="Generated automatically from your notes — edit freely.">
...
function NotesView({ onOpen, openNew }) {
  ...
  return <div>
    ...
    {filtered.length===0 ? <Empty icon="📝" title="No notes yet" sub="Capture your first conversation or thought — pick a type, add who was there, and write." action={<Btn onClick={()=>openNew()}>＋ New note</Btn>} />
      : <div className="grid gap-3 lg:grid-cols-2">{filtered.map(n=><NoteCard key={n.id} n={n} onOpen={onOpen} />)}</div>}
  </div>;
}

/* ============================================================================ */
/*  TIMELINE VIEW — who spoke to whom, when, in what form                        */
/* ============================================================================ */
function TimelineView({ onOpen }) {
  ...
  return <div>
    ...
    {sorted.length===0 ? <Empty icon="🕸️" title="No interactions yet" sub="Log a note with participants and it shows up here as part of the relationship timeline." />
  ...
  return <Modal open onClose={onClose} title="Note" wide>
    <div className="flex items-start justify-between gap-3">
      ...
      <div className="flex gap-1.5 shrink-0">
        ...
        <Btn variant="danger" size="sm" onClick={()=>{ if(confirm('Delete this note?')){removeNote(n.id);onClose();} }}>Delete</Btn></div>
    </div>
...
/* ============================================================================ */
/*  HOME (no gamification — status + recent activity only)                       */
/* ============================================================================ */
function Stat({ label, value, icon }) {
  return <div className="bg-white rounded-xl border border-slate-200 p-4 flex items-center gap-3"><div className="text-2xl">{icon}</div>
    <div><div className="text-2xl font-bold text-slate-800">{value}</div><div className="text-xs text-slate-500">{label}</div></div></div>;
}
function Home({ go, onOpen, openNew }) {
  const { data, me } = useApp();
  const recent = useMemo(()=>[...data.notes].sort((a,b)=>(b.datetime||'').localeCompare(a.datetime||'')).slice(0,6), [data.notes]);
  const thisWeek = data.notes.filter(n=>{ const d=new Date(n.datetime); return (Date.now()-d.getTime())<7*864e5; }).length;
  const attachments = data.notes.reduce((a,n)=>a+((n.attachments||[]).length),0);
  ...
  return <div className="space-y-6">
    <div className="rounded-2xl bg-gradient-to-br from-honey-400 to-honey-600 text-white p-6 shadow-lg">
      ...
      <h1 className="text-2xl font-extrabold mt-1">Hivemind</h1>
      <p className="text-honey-50/90 text-sm mt-1 max-w-lg">The shared brain of Bombay Bioworks. Take rich notes, log every conversation, and keep the collective memory of who we're talking to — searchable for everyone.</p>
      <div className="flex gap-2 mt-4"><Btn variant="dark" onClick={()=>openNew()}>＋ New note</Btn><Btn variant="ghost" onClick={()=>go('people')}>Add a contact</Btn></div>
    </div>
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      ...
      <Stat icon="📎" label="Attachments" value={attachments} />
    </div>
    <div className="grid lg:grid-cols-3 gap-4">
      <div className="lg:col-span-2">
        <div className="flex items-center justify-between mb-2"><h3 className="font-bold text-slate-700">Recent notes</h3><button onClick={()=>go('notes')} className="text-sm text-honey-600 hover:underline">View all →</button></div>
      ...
      <div className="space-y-4">
        <div><h3 className="font-bold text-slate-700 mb-2">By type</h3>
        ...
        <div><h3 className="font-bold text-slate-700 mb-2">Jump in</h3>
          <div className="bg-white rounded-xl border border-slate-200 p-3 grid grid-cols-2 gap-2">
            ...
            <Btn variant="ghost" size="sm" onClick={()=>openNew('note')}>📝 Quick note</Btn>
...
function SettingsView() {
  ...
  const exportData = () => { const blob=new Blob([JSON.stringify({people:data.people,notes:data.notes,exported:nowISO()},null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='hivemind-backup-'+new Date().toISOString().slice(0,10)+'.json'; a.click(); };
  const importData = async (file) => { const j=JSON.parse(await file.text()); for(const p of j.people||[]) await store.put('people',p); for(const n of j.notes||[]) await store.put('notes',n); await reload(); alert('Imported '+((j.people||[]).length)+' people, '+((j.notes||[]).length)+' notes.'); };
  return <div className="max-w-2xl space-y-6">
    <h2 className="text-xl font-bold text-slate-800">Settings</h2>
    <section className="bg-white rounded-xl border border-slate-200 p-5">
      <h3 className="font-bold text-slate-700 mb-3">Your identity</h3>
      <Field label="Your name" hint="Stamped on everything you log so the hive knows who captured what.">
    ...
    <section className="bg-white rounded-xl border border-slate-200 p-5">
      ...
      <p className="text-sm text-slate-500 mb-3">By default data lives in this browser only. To share one hive brain across the team, connect a free Supabase project (README has the 2-minute setup + SQL).</p>
...
function App() {
  ...
  const [data, setData] = useState({ people:[], notes:[] });
  const [loading, setLoading] = useState(true);
  const [openPerson, setOpenPerson] = useState(null);
  const [openNoteId, setOpenNoteId] = useState(null);
  const [editNote, setEditNote] = useState(null); // note object, or { __new:true, type }
  const [search, setSearch] = useState('');

  const reload = useCallback(async () => {
    setLoading(true);
    try { const [people, notes] = await Promise.all([store.list('people'), store.list('notes')]); setData({ people, notes }); }
  ...
  const openNew = (type) => setEditNote({ __new:true, type });
  const ctx = { cfg, setConfig, store, me, data, reload, savePerson, removePerson, saveNote, removeNote };

  const results = useMemo(()=>{ if(!search.trim()) return null; const q=search.toLowerCase();
    return { people:data.people.filter(p=>(p.name+p.org+p.role+(p.tags||[]).join('')).toLowerCase().includes(q)).slice(0,5),
             notes:data.notes.filter(n=>(n.title+n.summary+stripHtml(n.body)+n.context).toLowerCase().includes(q)).slice(0,5) }; }, [search, data]);
    ...
    <div className="min-h-screen flex flex-col">
        ...
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center gap-3">
          ...
          <div className="flex-1 max-w-md mx-auto relative">
            ...
            {results && <div className="absolute mt-1 w-full bg-white rounded-xl border border-slate-200 shadow-lg overflow-hidden z-50 max-h-96 overflow-y-auto">
              {results.notes.length===0 && results.people.length===0 && <div className="p-3 text-sm text-slate-400">No matches.</div>}
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 py-6">
        {loading ? <div className="text-center py-20 text-honey-600">Loading the hive…</div> : <>
          {tab==='home' && <Home go={setTab} onOpen={setOpenNoteId} openNew={openNew} />}
          {tab==='notes' && <NotesView onOpen={setOpenNoteId} openNew={openNew} />}
      ...
      <footer className="text-center text-xs text-slate-400 py-6">🧠 Hivemind · collective intelligence for Bombay Bioworks · {store.mode==='cloud'?'cloud-synced':'local mode — connect Supabase in Settings to share'}</footer>

      {openPerson && <PersonDetail id={openPerson} onClose={()=>setOpenPerson(null)} onOpenNote={setOpenNoteId} />}
      {openNoteId && <NoteDetail id={openNoteId} onClose={()=>setOpenNoteId(null)} onEdit={setEditNote} />}
